...
# Indices from 2 to 100 (since non-zero values start from index 2)
indices = np.arange(1, max_iterations+1)
print(array)
# Plot the non-zero values
plt.plot(indices, array)
plt.title("Error Diagram of Fixed Point Iteration (b=1-x^2)")
plt.xlabel("Number of Iterations")
plt.ylabel("log10(error)")
plt.xticks(np.arange(1, max_iterations+1))
plt.show()
