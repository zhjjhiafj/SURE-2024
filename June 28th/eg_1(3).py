...
n=1.1
def q(u):
    return ((2.0**n*(n+2.0))**(1.0/(2.0*n+2.0)))*(1.0-((x_values**2)**0.5)**(1.0+1.0/n))**(n*1.0/(2.0*n+2.0))
num_intervals = 2000
start_point = -1.0
end_point = 1.0
...
#u_ufl = (2**n*(n+2))**(1.0/(2*n+2))*(1-((x[0]**2)**1/2)**(1.0+1.0/n))**((2*n+2.0)/n)

V = dolfinx.fem.functionspace(domain, ("Lagrange", 1))
u_D = dolfinx.fem.Function(V)
u_D.interpolate(lambda x: np.zeros_like(x[0]))
u_exact = dolfinx.fem.Function(V)
u_exact.interpolate(lambda x: 2 * (n + 2) ** (1.0 / n) * (1 - abs(x[0]) ** (1.0 + 1.0 / n)))
...
uh.interpolate(lambda x: 2*(n+2)**(1.0/n)*(1-((x[0]**2)**0.5)**(1.0+1.0/n)))
...
i = 0
error = dolfinx.fem.form(ufl.inner(uh - u_exact, uh - u_exact) * ufl.dx(metadata={"quadrature_degree": 4}))
L2_error = []
du_norm = []
max_iterations = 50
...
print(uh.x.array[200])
print(u_exact.x.array[200])
...
if domain.comm.rank == 0:
    plt.figure()
    plt.plot(x_values, u_values**(n/(2*n+2.0)), label="Numerical solution")
