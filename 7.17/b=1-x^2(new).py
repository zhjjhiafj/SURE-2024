...
max_iterations = 30 # Optional: to prevent infinite loops
iteration = 0
uh = fem.Function(V)
...
while error > tolerance and iteration < max_iterations:
    ...
    A = ((n / (2.0 * n + 2.0)) ** n) / (n + 2.0) * (ufl.dot(ufl.grad(uh2) - Phi, ufl.grad(uh2) - Phi)) ** (
                (n - 1.0) / 2.0)
    alpha=1
...
u_e =fem.Function(V)
u2 = fem.Expression(u_ex, V.element.interpolation_points())
u_e.interpolate(u2)
...
error_max = numpy.log(domain.comm.allreduce(np.max((uh.x.array)**(n/(2*n+2.0)) - u_e.x.array), op=MPI.MAX))
print(error_L2)
print(error_max)
...
if domain.comm.rank == 0:
   plt.figure()
   plt.grid()
   plt.plot(x_values, u_values+1-x_values**2, label="Numerical Solution")
   plt.plot(x_values, 1-x_values**2, label="b(x)")
   plt.xlabel("x")
   plt.ylabel("H")
   plt.legend()
   plt.title("n=3, b=1-x^2")
